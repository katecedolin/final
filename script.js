...
runBtn.addEventListener('click', async () => {
  try {
    ...
    alert(err.message || 'Something went wrong.');
      ...
      // === Make data available for the SMS scheduler ===
    // Convert your internal data into the structure schedule.js expects.
    const drivers = [];
    const selfDrivers = [];
    const riders = [];
    ...
    // 2. Drivers with passengers
    for (const group of carpools) {
      if (!group.length || group.every(p => p.transportation === 1)) continue;
